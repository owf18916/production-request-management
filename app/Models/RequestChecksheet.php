...
class RequestChecksheet extends Model
{
    ...
    public static function countByStatus(string $status): int
    {
        if ($status === 'pending') {
            ...
            $sql = "SELECT COUNT(*) as count FROM request_checksheet WHERE approved_at IS NULL AND status != 'rejected' AND status != 'completed'";
            $result = Database::row($sql);
        } elseif ($status === 'approved') {
            // Approved: has approved_at timestamp
            $sql = "SELECT COUNT(*) as count FROM request_checksheet WHERE approved_at IS NOT NULL";
            $result = Database::row($sql);
        } else {
