    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Request ID</h1>
                    <p class="mt-2 text-gray-600">Manage your ID requests</p>
                </div>
                <a href="<?php echo url('request-id/create'); ?>" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700">
            ...
            <form method="GET" class="space-y-4">
                <div class="flex gap-4 flex-wrap">
                    <div class="flex-1 min-w-xs">
                        <input type="text" name="search" placeholder="Search by request number..." value="<?php echo htmlspecialchars($search ?? ''); ?>" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <select name="id_type" class="px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="flex gap-4 flex-wrap items-end">
                    ...
                    <a href="<?php echo url('request-id?clear_filters=1'); ?>" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">Reset</a>
                </div>
            </form>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID Type</th>
                        ...
                        <?php foreach ($requests as $request): ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-mono text-sm font-medium text-gray-900"><?php echo htmlspecialchars($request->request_number); ?></span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    ?>
                                    <span class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $statusColors[$request->status] ?? 'bg-gray-100 text-gray-800'; ?>">
                                        <?php echo htmlspecialchars($statusLabels[$request->status] ?? ucfirst($request->status)); ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="<?php echo url('request-id/' . $request->id); ?>" class="text-blue-600 hover:text-blue-900">View</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
        ...
        <div class="mt-6 flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <nav class="mt-4 md:mt-0 flex items-center justify-center space-x-2">
                ...
                <?php if ($pagination->hasPreviousPage()): ?>
                    <a href="<?php echo url('request-id?page=' . $pagination->getPreviousPage() . '&search=' . urlencode($search ?? '') . '&id_type=' . urlencode($idTypeFilter ?? '') . '&status=' . urlencode($statusFilter ?? '') . '&start_date=' . urlencode($startDate ?? '') . '&end_date=' . urlencode($endDate ?? '')); ?>" 
                ...
                <div class="flex items-center space-x-1">
                    ...
                    if ($pageRange[0] > 1): ?>
                        <a href="<?php echo url('request-id?page=1&search=' . urlencode($search ?? '') . '&id_type=' . urlencode($idTypeFilter ?? '') . '&status=' . urlencode($statusFilter ?? '') . '&start_date=' . urlencode($startDate ?? '') . '&end_date=' . urlencode($endDate ?? '')); ?>" 
                           class="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                            1
                        </a>
                    ...
                    <?php foreach ($pageRange as $page): ?>
                        ...
                        <?php else: ?>
                            <a href="<?php echo url('request-id?page=' . $page . '&search=' . urlencode($search ?? '') . '&id_type=' . urlencode($idTypeFilter ?? '') . '&status=' . urlencode($statusFilter ?? '') . '&start_date=' . urlencode($startDate ?? '') . '&end_date=' . urlencode($endDate ?? '')); ?>" 
                               class="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                                <?php echo $page; ?>
                            </a>
                        <?php endif; ?>
                    ...
                    <?php if ($pageRange[count($pageRange) - 1] < $pagination->getTotalPages()): ?>
                        <?php if ($pageRange[count($pageRange) - 1] < $pagination->getTotalPages() - 1): ?>
                            <span class="px-2 py-2 text-gray-700">...</span>
                        <?php endif; ?>
                        <a href="<?php echo url('request-id?page=' . $pagination->getTotalPages() . '&search=' . urlencode($search ?? '') . '&id_type=' . urlencode($idTypeFilter ?? '') . '&status=' . urlencode($statusFilter ?? '') . '&start_date=' . urlencode($startDate ?? '') . '&end_date=' . urlencode($endDate ?? '')); ?>" 
                           class="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                ...
                <?php if ($pagination->hasNextPage()): ?>
                    <a href="<?php echo url('request-id?page=' . $pagination->getNextPage() . '&search=' . urlencode($search ?? '') . '&id_type=' . urlencode($idTypeFilter ?? '') . '&status=' . urlencode($statusFilter ?? '') . '&start_date=' . urlencode($startDate ?? '') . '&end_date=' . urlencode($endDate ?? '')); ?>" 
                       class="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                        Next →
                    </a>
                <?php else: ?>
