...
<div class="py-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 md:ml-2 text-sm font-medium text-gray-500">Edit Conveyor</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Edit Conveyor</h1>
        <p class="text-gray-600 mt-1">Update conveyor information</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="<?php echo url("admin/master/conveyor/update/{$conveyor->id}"); ?>" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="conveyor_name" name="conveyor_name" 
                       value="<?php echo e($oldInput['conveyor_name'] ?? $conveyor->conveyor_name); ?>"
                ...
                <div class="space-y-2">
                    <div class="flex items-center">
                        <input type="radio" id="status_active" name="status" value="active" 
                               <?php $statusToCheck = $oldInput['status'] ?? $conveyor->status; echo ($statusToCheck === 'active' ? 'checked' : ''); ?>
                    ...
                    <div class="flex items-center">
                        <input type="radio" id="status_inactive" name="status" value="inactive"
                               <?php $statusToCheck = $oldInput['status'] ?? $conveyor->status; echo ($statusToCheck === 'inactive' ? 'checked' : ''); ?>
                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                <p class="text-sm text-gray-600">
                    <span class="font-medium">Created By:</span> <?php echo e($conveyor->created_by_name); ?>
                </p>
                <p class="text-sm text-gray-600 mt-2">
                    <span class="font-medium">Created At:</span> <?php echo date('Y-m-d H:i:s', strtotime($conveyor->created_at)); ?>
                </p>
                <p class="text-sm text-gray-600 mt-2">
                    <span class="font-medium">Updated At:</span> <?php echo date('Y-m-d H:i:s', strtotime($conveyor->updated_at)); ?>
                </p>
            </div>
